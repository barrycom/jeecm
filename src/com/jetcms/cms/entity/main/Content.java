...
import static com.jetcms.common.web.Constants.SPT;
...
import com.jetcms.cms.Constants;
import com.jetcms.cms.entity.assist.CmsComment;
import com.jetcms.cms.entity.assist.CmsScoreRecord;
import com.jetcms.cms.entity.main.Channel.AfterCheckEnum;
import com.jetcms.cms.entity.main.base.BaseContent; 
import com.jetcms.cms.web.CmsThreadVariable;
...
import com.jetcms.core.entity.CmsWorkflowNode; 
import com.jetcms.core.web.ContentInterface;
...
public class Content extends BaseContent implements ContentInterface {
	...
	public String getUrl() {
		if (!StringUtils.isBlank(getLink())) {
			return getLink().replaceAll(":80/","/");
		}
		if (getStaticContent()) {
			return "";
		} else if(!StringUtils.isBlank(getSite().getDomainAlias())){
			return getUrlDynamic(null).replaceAll(":80/","/");
		}else{
			return getUrlDynamic(true).replaceAll(":80/","/");
		}
	}
	...
	public void addToCatalog(Integer id,String  pathpasss,String livepass,String livepath,String path,String name,  String lectureDate, Integer priority, 
			Integer viewCount,  String courseCategory, String startTime, String endTime) {
		List<ContentCatalog> list = getCatalog();
		if (list == null) {
			list = new ArrayList<ContentCatalog>();
			setCatalog(list);
		}
		ContentCatalog ca = new ContentCatalog(id,pathpasss,livepass,livepath,path, name, lectureDate, priority, viewCount, courseCategory, startTime, endTime);
		if (!StringUtils.isBlank(name)) {
			ca.setName(name);;
		}
		list.add(ca);
	}
	public void addToKj(Integer id, String path,String pathPass,String name,  Integer priority  ) {
		List<ContentKj> list = getKj();
		if (list == null) {
			list = new ArrayList<ContentKj>();
			setKj(list);
		}
		ContentKj ca = new ContentKj(id,path,pathPass, name, priority);
		if (!StringUtils.isBlank(name)) {
			ca.setName(name);;
		}
		list.add(ca);
	 ...
	 */
	public Content(Integer id,
			com.jetcms.core.entity.CmsSite site, Date sortDate,
